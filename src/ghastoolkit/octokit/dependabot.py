...
class Dependabot:
    ...
    def getAlerts(self) -> list[dict]:
        ...
        while True:
            data = self.graphql.query("GetDependencyAlerts", options={"owner": self.repository.owner, "repo": self.repository.repo})
            alerts = data.get("data", {}).get("repository", {}).get("vulnerabilityAlerts", {})
