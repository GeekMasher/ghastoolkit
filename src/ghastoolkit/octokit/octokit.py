...
class Octokit:
    ...
    def route(path: str, repository: Repository, rtype: str = "rest", **options) -> str:
        ...
        """
        logger.debug(f"Generating route for {rtype} API: path={path}, repository={repository.owner}/{repository.repo}")
    ...
    def formatPath(path: str, repo: Repository, **options) -> str:
        ...
        """
        logger.debug(f"Formatting path: {path} with repo={repo.owner}/{repo.repo} and options={options}")
...
def loadOctoItem(classtype: type, data: dict) -> OctoItem:
    ...
    if not (issubclass(classtype, OctoItem) and is_dataclass(classtype)):
        error_msg = f"Class should be a dataclass OctoItem subclass, got {classtype.__name__}"
    ...
    new.__data__ = data
    logger.debug(f"Created {classtype.__name__} instance with {len(initdata)} constructor fields and {len(data)} total data fields")
    return new
...
class RestRequest:
    ...
    def get(
        ...
        """
        logger.debug(f"GET request: path={path}, parameters={parameters}, expected={expected}, authenticated={authenticated}")
        ...
        while True:
            ...
            if cursor:
                ...
                logger.debug(f"Pagination: Using cursor {params['after']} for page {page_count}")
            else:
                params["page"] = page
            ...
            important_headers = ['X-RateLimit-Limit', 'X-RateLimit-Remaining', 'X-RateLimit-Reset', 
                                'X-GitHub-Request-Id', 'Link']
            headers_to_log = {k: v for k, v in response.headers.items() if k in important_headers}
            ...
            if isinstance(response_json, list):
                ...
                logger.debug(f"Received dictionary response with {len(response_json)} keys")
                if 'total_count' in response_json:
                    logger.debug(f"Response indicates total_count: {response_json['total_count']}")

            # Handle unexpected status codes
            if expected and response.status_code != expected:
                if display_errors:
                    logger.error(f"Error code from server: {response.status_code}, expected: {expected}")
                ...
                if known_error:
                    logger.error(f"Known error type: {type(known_error).__name__}, status: {known_error.status}")
                    raise known_error
                else:
            ...
            if isinstance(response_json, dict) and response_json.get("message"):
                ...
                if error_handler:
                    logger.debug("Calling error handler callback for response with error message")
                    return error_handler(response.status_code, response_json)
            ...
            logger.debug(f"Added {current_count} items from page {page_count}, total items so far: {total_items}")
            ...
            if link := response.headers.get("Link"):
                ...
                if len(response_json) >= RestRequest.PER_PAGE:
                    page += 1
                    logger.debug(f"Received full page of results, incrementing page to {page}")
                else:
                    logger.debug(f"Received partial page ({len(response_json)} items), ending pagination")
                    break

        logger.debug(f"Pagination complete: retrieved {len(result)} total items across {page_count} pages")
        return result
...
class GraphQLRequest:
    ...
    def query(self, name: str, options: dict[str, Any] = {}) -> dict:
        ...
        if not query_content:
            ...
            logger.debug(f"Found query '{name}', length: {len(query_content)} characters")
        ...
        safe_options = {k: v if k not in ['token', 'password'] else '***' for k, v in options.items()}
        ...
        if rjson.get("errors"):
            ...
            for i, err in enumerate(rjson.get("errors")):
                ...
                loc_str = ", ".join(f"line {l.get('line', '?')}, col {l.get('column', '?')}" 
                                  for l in locations) if locations else "unknown location"
                
                logger.warning(f"GraphQL error {i+1}/{error_count}: {message} at {path_str} ({loc_str})")
        ...
        if "data" in rjson:
            if rjson["data"] is None:
                ...
                data_keys = rjson["data"].keys() if isinstance(rjson["data"], dict) else []
                ...
                for file in files:
                    ...
                    try:
                        with open(root, "r") as handle:
                            data = handle.read()
                        logger.debug(f"Loaded GraphQL Query: {name} ({len(data)} characters)")
    ...
    def formatQuery(self, query: str, **options) -> str:
        ...
        if missing_vars:
            logger.warning(f"Potential missing variables in GraphQL query: {', '.join(missing_vars)}")
            
        # Log extra variables that won't be used
        extra_vars = provided_vars - expected_vars
        if extra_vars:
            logger.debug(f"Extra variables provided but not used in query: {', '.join(extra_vars)}")
