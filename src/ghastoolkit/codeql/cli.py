...
class CodeQL:
    ...
    def runQuery(self, database: CodeQLDatabase, path: Optional[str] = None) -> CodeQLResults:
    ...
    def runRawQuery(self, path: str, database: CodeQLDatabase) -> list:
        ...
        self.runCommand("database", "run-queries", database.path, path)
        bqrs = os.path.join(database.path, "results", path.replace(":", "/").replace(".ql", ".bqrs"))
        return self.readBqrs(bqrs).get("#select", {}).get("tuples", [])

    def getResults(self, database: CodeQLDatabase, path: Optional[str] = None) -> CodeQLResults:
    ...
    def readBqrs(self, bqrsfile: str) -> dict:
        ...
        self.runCommand("bqrs", "decode", "--format", "json", "--output", output, bqrsfile)
